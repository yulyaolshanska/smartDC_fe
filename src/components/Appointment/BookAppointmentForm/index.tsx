...
const BookAppointmentForm: React.FC = () => {
    ...
    const onSubmit = async (data) => {
        ...
        const start = parse(`${format(data.date, 'yyyy-MM-dd')} ${startTime.trim()}`, 'yyyy-MM-dd hh:mm aa', new Date());
        const end = parse(`${format(data.date, 'yyyy-MM-dd')} ${endTime.trim()}`, 'yyyy-MM-dd hh:mm aa', new Date());
